        ...
        for attempt in range(retries):
            try:
                ...
                if meta_tag:
                    ...
                    # Example content: "276K likes, 1,328 comments - sufitramp on June 6, 2025: ..."
                    # Extract likes, comments, and date using regex
                    match = re.search(r"([\d,.MK]+)\s*likes,\s*([\d,.MK]+)\s*comments\s*-\s*\w+\s*on\s*([A-Za-z]+\s*\d{1,2},\s*\d{4})", content, re.IGNORECASE)
                    if match:
                        data["Likes"] = parse_number(match.group(1))  # e.g., "276K"
                        data["Comments"] = parse_number(match.group(2))  # e.g., "1,328"
                        data["Upload_Date"] = parse_date(match.group(3))  # e.g., "June 6, 2025"
